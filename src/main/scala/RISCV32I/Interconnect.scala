...
class Interconnect(MASTERS: Int, SLAVES: Int, DATA_BUS_SIZE: Int, ADDR_BUS_SIZE: Int) extends Module {
    ...
    for(i <- 0 until SLAVES) {
        // read address channel
        io.M_AXI_ARREADY(arbuiter_rd + i)   := io.S_AXI_ARREADY(i)
        io.S_AXI_ARVALID(i)                 := io.M_AXI_ARVALID(arbuiter_rd + i)
        io.S_AXI_ARADDR(i)                  := io.M_AXI_ARADDR(arbuiter_rd + i)
        io.S_AXI_ARPROT(i)                  := io.M_AXI_ARPROT(arbuiter_rd + i)

        // read data channel
        io.S_AXI_RREADY(i)                  := io.M_AXI_RREADY(arbuiter_rd + i)
        io.M_AXI_RVALID(arbuiter_rd + i)    := io.S_AXI_RVALID(i)
        io.M_AXI_RDATA(arbuiter_rd + i)     := io.S_AXI_RDATA(i)
        io.M_AXI_RRESP(arbuiter_rd + i)     := io.S_AXI_RRESP(i)

        // write address channel
        io.M_AXI_AWREADY(arbuiter_wr + i)   := io.S_AXI_AWREADY(i)
        io.S_AXI_AWVALID(i)                 := io.M_AXI_AWVALID(arbuiter_wr + i)
        io.S_AXI_AWADDR(i)                  := io.M_AXI_AWADDR(arbuiter_wr + i)
        io.S_AXI_AWPROT(i)                  := io.M_AXI_AWPROT(arbuiter_wr + i)

        // write data channel
        io.M_AXI_WREADY(arbuiter_wr + i)    := io.S_AXI_WREADY(i)
        io.S_AXI_WVALID(i)                  := io.M_AXI_WVALID(arbuiter_wr + i)
        io.S_AXI_WDATA(i)                   := io.M_AXI_WDATA(arbuiter_wr + i)
        io.S_AXI_WSTRB(i)                   := io.M_AXI_WSTRB(arbuiter_wr + i)

        // write response channel
        io.S_AXI_BREADY(i)                  := io.M_AXI_BREADY(arbuiter_wr + i)
        io.M_AXI_BVALID(arbuiter_wr + i)    := io.S_AXI_BVALID(i)
        io.M_AXI_BRESP(arbuiter_wr + i)     := io.S_AXI_BRESP(i)
    }

    for(i <- 0 until SLAVES) {
        units_stat_rd(i)    := ~((io.S_AXI_ARREADY(i) ^ io.M_AXI_ARVALID(arbuiter_rd + i)) | (io.M_AXI_RREADY(arbuiter_rd + i) ^ io.S_AXI_RVALID(i)))
        units_stat_wr(i)    := ~((io.S_AXI_BVALID(i) ^ io.M_AXI_BREADY(arbuiter_wr + i)) | (io.S_AXI_WREADY(i) ^ io.M_AXI_WVALID(arbuiter_wr + i)) | (io.M_AXI_AWVALID(arbuiter_wr + i) ^ io.S_AXI_AWREADY(i)))
    }

    when(units_stat_rd.andR()) {
        arbuiter_rd     := arbuiter_rd + 1
    }
    when(units_stat_wr.andR()) {
        arbuiter_wr     := arbuiter_wr + 1
    }
}